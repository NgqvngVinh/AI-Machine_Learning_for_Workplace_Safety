...
#     def update_view(self, _image, _objs, metadata_by_id):       # _objs of type sl.Objects
#         self.mutex.acquire()
...
#         #         color_id = generate_color_id(obj.id)  # Assuming this function exists and works as intended

#         #         # Retrieve metadata for each object
#         #         real_name, occupation, birthday = metadata_by_id.get(obj.id, ("Unknown", "Unknown", "Unknown"))

#         #         # Create rendering with full information
#         #         self.create_id_rendering(pos, color_id, obj.id, real_name, occupation, birthday)
#         # ################################################################
...
#                 #############
#                 real_name, occupation, birthday =metadata_by_id.get(id, ("Unknown", "Unknown", "Unknown"))
#                 #############
...
#                         self.create_id_rendering(pos, color_id, _objs.object_list[i].id, real_name, occupation, birthday)
...
#     def create_id_rendering(self, _center, _clr, _id, real_name, occupation, birthday):
#         tmp = ObjectClassName()
#         # tmp.name = "ID: " + str(_id)
#         ###dong duoi them de set data cho object###
#         tmp.set_metadata(real_name, occupation, birthday)
#         tmp.name = f"ID: " + str(_id), (real_name), (occupation), (birthday)
#         ################################
#         tmp.color = _clr
...
#                 ###add##
#                 full_text = f"{obj.name} - {obj.occupation} - {obj.birthday}"
...
#     def set_metadata(self, real_name, occupation, birthday):
...
class GLViewer:
    ...
    def __init__(self):
        ...
        self.polygons = [[[-0.23 ,0.95,  -5],[-0.53, -2.7,-5]],[[-0.23 ,0.95,  -5],[-5, 0.85,-5]],[[1.75 ,-0.12,  -5],[3.62, -3,-5]]]

        self.safe_zone_1 = {"a":36.5/3, "b":0.95+8.395/3,"color":[0,255,0,255]}
        self.safe_zone_2 = {"a":-288/187, "b":-0.12+504/187,"color":[0,255,0,255]}
    def init(self, _params, _is_tracking_on):
        glutInit()
        wnd_w = glutGet(GLUT_SCREEN_WIDTH)
        wnd_h = glutGet(GLUT_SCREEN_HEIGHT)
        ...
        ########
        self.safe_area = Simple3DObject(False)
        self.safe_area.set_drawing_type(GL_LINES)
        #########
    ...
    def update_view(self, _image, _objs, metadata_by_id):       # _objs of type sl.Objects
        ...
        self.BBox_edges.clear()
        self.BBox_faces.clear()
        self.safe_area.clear()
        self.objects_name = []

        for i in range(len(_objs.object_list)):
            if self.render_object(_objs.object_list[i]):
                #############
                real_name, occupation, age, birthday =metadata_by_id.get(_objs.object_list[i].id, ("Hung", "Software Developer", "20", "2004-01-01"))
                #############
                bounding_box = np.array(_objs.object_list[i].bounding_box)
                if bounding_box.any():
                    color_class = get_color_class(0)
                    ...
                    if _objs.object_list[i].tracking_state != sl.OBJECT_TRACKING_STATE.OK:
                        ...
                        pos = [_objs.object_list[i].position[0], _objs.object_list[i].bounding_box[0][1], _objs.object_list[i].position[2]]
                        self.create_id_rendering(pos, color_id, _objs.object_list[i].id, real_name, occupation, age, birthday)

                    self.create_bbox_rendering(bounding_box, color_id)
                    x = bounding_box[6][0] #6
                    y = bounding_box[6][1] #6
                    
                    if y -(self.safe_zone_1["a"]*x-self.safe_zone_1["b"])>3:
                        self.safe_zone_1["color"] = [255,0,0,255]

                    # if y -(self.safe_zone_2["a"]*x-self.safe_zone_2["b"])<-1:
                    #     self.safe_zone_2["color"] = [255,0,0,255]
        ############# draw line limit #########################

        self.safe_area.add_line(self.polygons[0][0],self.polygons[0][1],self.safe_zone_1["color"])
        self.safe_area.add_line(self.polygons[1][0],self.polygons[1][1],self.safe_zone_1["color"])
        self.safe_area.add_line(self.polygons[2][0],self.polygons[2][1],self.safe_zone_2["color"])
    ...
    def create_id_rendering(self, _center, _clr, _id, real_name, occupation, age, birthday):
        tmp = ObjectClassName()
        # tmp.name = "ID: " + str(_id)
        ###dong duoi them de set data cho object###
        tmp.set_metadata(real_name, occupation, age, birthday)
        # tmp.name = f"ID: " + str(_id), (real_name), (occupation), (birthday)
        tmp.name = f"ID: " + str(_id)
        tmp.real_name = str(real_name)
        tmp.occupation = str(occupation)
        tmp.age = str(age)
        tmp.birthday = str(birthday)
        ################################
        tmp.color = _clr
        tmp.position = np.array([_center[0], _center[1], _center[2]], np.float32)
        self.objects_name = np.append(self.objects_name, tmp)
    ...
    #         op.glReadBuffer(op.GL_FRONT)
    #         data = op.glReadPixels(0, 0, 1280, 720, op.GL_BGR, op.GL_UNSIGNED_BYTE)
    ...
    def print_text(self):
        ...
        if len(self.objects_name) > 0:
            for obj in self.objects_name:
                ...
                glWindowPos2f(pt2d[0], pt2d[1])
                ###add##
                base_x = pt2d[0]  # Set base_x from projected x-coordinate
                base_y = pt2d[1]  # Set base_y from projected y-coordinate
                line_height = 18
                # full_text = f"{obj.name}, {obj.real_name}, {obj.occupation}, {obj.birthday}"
                # for char in full_text:
                #     glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, ctypes.c_int(ord(char)))
                info_lines = [
                    f"{obj.name}",
                    f"Name: {obj.real_name}",
                    f"Occupation: {obj.occupation}",
                    f"Age: {obj.age}",
                    f"Birthday: {obj.birthday}"
                ]
                for i, line in enumerate(info_lines):
                    glWindowPos2f(base_x, base_y - i * line_height)
                    for char in line:
                        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, ctypes.c_int(ord(char)))
...
class ObjectClassName:
    ...
    def set_metadata(self, real_name, occupation, age, birthday):
        self.real_name = real_name
        self.occupation = occupation
        self.age = age
        self.birthday = birthday
