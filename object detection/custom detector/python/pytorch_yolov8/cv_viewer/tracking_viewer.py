...
def update_view(self, image, objects, point_cloud):
    self.clear_view()
    self.draw_image(image)
    for obj in objects.object_list:
        # Vẽ hộp giới hạn 3D
        if obj.tracking_state == sl.OBJECT_TRACKING_STATE.OK:
            # Tính toán vị trí 3D từ point cloud
            top_left_3d = self.get_3d_coordinates(point_cloud, obj.bounding_box_2d[0])
            bottom_right_3d = self.get_3d_coordinates(point_cloud, obj.bounding_box_2d[2])

            # Vẽ hộp giới hạn 3D
            self.draw_3d_box(top_left_3d, bottom_right_3d, obj.label)
    self.refresh_view()

def get_3d_coordinates(self, point_cloud, pixel_coordinates):
    x, y = pixel_coordinates
    point3D = point_cloud.get_value(x, y)
    return point3D[0:3]  # Trả về tọa độ X, Y, Z

def draw_3d_box(self, top_left_3d, bottom_right_3d, label):
    # Giả sử đây là phương thức vẽ hộp 3D dựa trên tọa độ 3D
    print(f"Drawing 3D box for {label} at {top_left_3d} to {bottom_right_3d}")
